...
class Laplacian(torch.autograd.Function):
    def __init__(self, faces,V):
        ...
        if self.L is None:
            ...
            Apt = torch.sparse.FloatTensor(torch.LongTensor([Acoo.row.tolist(), Acoo.col.tolist()]),
                              torch.FloatTensor(Acoo.data.astype(np.float))).cuda()
            #L=L.cuda()
            # remember this
            Apt.float()
            self.L = Apt
    ...
    def forward(ctx, V,L):
        ...
        #print(batchV.dtype)
        #print(L.dtype)
        Lx = torch.mm(L,batchV).reshape(V.shape)
        ctx.save_for_backward(V,L)
        return Lx
    @staticmethod
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            grad_input = L.mm(g_o).reshape(grad_output.shape)
        #if ctx.needs_input_grad[1]:
            #grad_weight = grad_output.t().mm(V)
